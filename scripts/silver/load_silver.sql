...
Purpose: 
    This stored procedure performs data transformation and cleansing 
    from the Bronze Layer into the Silver Layer of the data warehouse. 
    It refines, validates, and standardizes CRM and ERP datasets before 
    they are made available for analytical processing. Specifically, it:
        - Truncates Silver tables before reload
        - Cleans and transforms raw data from the Bronze Layer
        - Applies data quality rules (type casting, trimming, validation)
        - Handles nulls, invalid values, and inconsistent formatting
        - Ensures referential consistency between CRM and ERP datasets
